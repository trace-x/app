...
{
    style_sheet.replace(QRegularExpression(QString("\\$%1\\$").arg(key)), value);
}
...
int main(int argc, char *argv[])
{
    ...
    {
        QTextStream(stdout) << "Trace-X version: " << Settings::version() << Qt::endl;
    ...
    else
    {
        ...
        {
            QTextStream(stdout) << "\nTrace server [" << service.trace_server().server_name() << "] started at : " << QTime::currentTime().toString() << Qt::endl;
...
//                QTextStream(stdout) << "Saving trace: " << service.trace_controller().trace_model().safe_size() << " messages" << Qt::endl;

//                service.trace_controller().save_trace(trace_file_name);

//                QTextStream(stdout) << "Trace stored on:" << QFileInfo(trace_file_name).absoluteFilePath() << Qt::endl;
//            }
        ...
        {
            QTextStream(stderr) << "service is not started" << Qt::endl;
        }
    }
